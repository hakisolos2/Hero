...
const connect = async () => {
  const initializeConnection = async () => {
    try {
      ...
      const credsPath = path.join(__dirname, "session", "creds.json");  // Use path.join for creds.json

      if (fs.existsSync(credsPath)) {
        await fs.unlinkSync(credsPath);  // Remove old creds.json
      }

      if (!fs.existsSync(credsPath)) {
        await fs.writeFileSync(credsPath, sessionID);  // Write new creds.json
      }

      const { state, saveCreds } = await useMultiFileAuthState(path.join(__dirname, "session"));  // Path for auth state
