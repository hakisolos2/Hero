...
const connect = async () => {
  const _0x232309 = async () => {
    try {
      ...
      if (!fs.existsSync(__dirname + "/session/creds.json")) {
        await fs.writeFileSync(__dirname + "/session/creds.json", _0x5e951f);
      }
      ;
      const {
        state: _0x39bf9d,
