...
const connect = async () => {
  const initializeConnection = async () => {
    try {
      ...
      // Adjust the path to point to the session folder outside the script's directory
      const credsFilePath = path.resolve(__dirname, "../session/creds.json");

      // Check if creds.json exists in the external session folder
      if (fs.existsSync(credsFilePath)) {
        await fs.unlinkSync(credsFilePath);
      }

      // If creds.json doesn't exist, create it with the sessionID
      if (!fs.existsSync(credsFilePath)) {
        await fs.writeFileSync(credsFilePath, sessionID);
      }

      // Adjust the auth state to use the external session folder
...
await socket.sendMessage(socket.user.id, {
  ...
  try {
    ...
    console.error("NEXUS-BOT function error:", error);
  }
};
